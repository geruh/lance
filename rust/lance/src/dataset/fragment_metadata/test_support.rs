...
use lance_table::fragment_metadata::{FragmentTree, TouchedFragments};
...
pub(super) async fn execute_add_columns(
    ...
    let messages = actions
        .into_iter()
        .enumerate()
        .map(|(action_sequence, action)| pb::FragmentTreeMutation {
...
impl Reader {
    ...
    async fn open_uri_version(uri: &str, version: Option<u64>) -> Result<Self> {
        ...
        let tree =
            tree_from_manifest(object_store, session.store_registry(), base, &manifest).await?;
        Ok(Self {
            tree,
