...
use lance_table::fragment_metadata::node::{apply_verified, squash_buffer};
...
#[tokio::test]
async fn replacement_can_change_file_version_like_flat() {
    ...
    assert_ne!(
        (base.file_major_version, base.file_minor_version),
        (major, minor)
    );
    let mut diff = Differential::create(N).await;
    let outcome = diff
    ...
    let flat = crate::Dataset::open(&diff.flat.uri).await.unwrap();
    let tree = crate::Dataset::open(&diff.fragment_metadata.uri)
        .await
        .unwrap();
    let mixed = lance_table::feature_flags::FLAG_MIXED_DATA_FILE_VERSIONS;
    assert_eq!(flat.manifest.reader_feature_flags & mixed, mixed);
    assert_eq!(flat.manifest.writer_feature_flags & mixed, mixed);
    assert_eq!(tree.manifest.reader_feature_flags & mixed, mixed);
    assert_eq!(tree.manifest.writer_feature_flags & mixed, mixed);
    assert_eq!(
        tree.manifest.data_storage_format,
        flat.manifest.data_storage_format
    );
    let state = diff.fragment_metadata_state().await;
    let stored = &state.fragments[&42].files[0];
    assert_eq!(
        (stored.file_major_version, stored.file_minor_version),
        (major, minor)
    );
}

#[tokio::test]
async fn replacement_with_unrecognized_file_version_fails_its_own_commit() {
    let mut file = make_fragment(42).files[0].clone();
    file.path = "replacement-unknown-version.lance".into();
    file.file_major_version = 17;
    file.file_minor_version = 42;
    invalid_replacement_fails_its_own_commit(
        "replace_unrecognized_file_version",
        Operation::DataReplacement {
            replacements: vec![DataReplacementGroup(42, file)],
        },
    )
...
async fn add_columns_updates_schema_and_files_atomically_like_flat() {
    ...
    // A backfill buffers one appended file per fragment, not whole records.
    assert_eq!(
        buffered_kinds(&diff.fragment_metadata.uri).await,
        BTreeMap::from([("add_data_file", 10)])
    );
...
async fn transaction_history_is_owned_by_version_manifest() {
    ...
    for (step, operation) in [
        ...
        assert!(dataset.manifest.fragment_tree.is_some());
        assert!(
...
/// Tree and flat commits both reject duplicate fragment IDs.
#[rstest::rstest]
#[case::append_with_a_live_id("append")]
#[case::two_replacements_of_one_fragment("replace")]
#[tokio::test]
async fn duplicate_fragment_ids_are_rejected_like_flat(#[case] operation: &str) {
    let mut diff = Differential::create(10).await;
    let operation = match operation {
        "append" => {
            let mut colliding = make_fragment(3);
            colliding.files[0].path = "collide.lance".to_string();
            colliding.physical_rows = Some(100);
            Operation::Append {
                fragments: vec![colliding],
            }
        }
        _ => {
            let mut first = make_backfill_data_file(3, 0);
            first.path = "first.lance".to_string();
            let mut second = make_backfill_data_file(3, 0);
            second.path = "second.lance".to_string();
            Operation::DataReplacement {
                replacements: vec![
                    DataReplacementGroup(3, first),
                    DataReplacementGroup(3, second),
                ],
            }
        }
    };
    let outcome = diff.apply_latest("duplicate_id", operation).await;
    assert!(
        outcome.both_rejected(),
        "flat={:?} fragment_metadata={:?}",
        outcome.flat.as_ref().err(),
        outcome.fragment_metadata.as_ref().err()
    );
    let reader = diff.fragment_metadata_reader().await;
    assert_eq!(
        reader.resolve_fragment(3).await.unwrap(),
        Some(make_fragment(3))
    );
}

#[rstest::rstest]
#[case::explicit_id("explicit_id")]
#[case::reservation("reservation")]
#[case::append_after_exhaustion("append_after_exhaustion")]
#[case::overwrite_after_exhaustion("overwrite_after_exhaustion")]
#[tokio::test]
async fn fragment_id_exhaustion_is_rejected_like_flat(#[case] operation: &str) {
    let mut diff = Differential::create(2).await;
    let operation = match operation {
        "explicit_id" => Operation::Append {
            fragments: vec![make_fragment(u64::from(u32::MAX) + 1)],
        },
        "reservation" => Operation::ReserveFragments {
            num_fragments: u32::MAX,
        },
        _ => {
            let reserved = diff
                .apply_latest(
                    "reserve_remaining_ids",
                    Operation::ReserveFragments {
                        num_fragments: u32::MAX - 1,
                    },
                )
                .await;
            assert!(
            ...
            if operation == "overwrite_after_exhaustion" {
                Operation::Overwrite {
                    fragments: vec![make_fragment(0)],
    ...
    let outcome = diff.apply_latest("exhaust_fragment_ids", operation).await;
    for result in [outcome.flat, outcome.fragment_metadata] {
        let error = result.unwrap_err();
        assert!(matches!(error, Error::InvalidInput { .. }), "{error}");
        assert!(error.to_string().contains("u32 address space"), "{error}");
    }
    assert!(diff.states_equal().await);
}

#[tokio::test]
async fn appending_modern_files_to_legacy_storage_is_rejected_like_flat() {
    let fragments = (0..2)
        .map(|id| {
            let mut fragment = make_fragment(id);
            let (major, minor) = LanceFileVersion::Legacy.resolve().to_data_file_numbers();
            fragment.files[0].file_major_version = major;
            fragment.files[0].file_minor_version = minor;
            fragment
        })
        .collect();
    let mut diff = Differential::create_with_fragments(fragments).await;
    let outcome = diff
        .apply_latest("append_modern_file", production_style_append(1))
        .await;
    assert!(
        outcome.both_rejected(),
        "flat={:?} tree={:?}",
...
#[case::tree(true)]
#[tokio::test]
async fn create_rejects_invalid_column_mappings(#[case] tree: bool) {
    let dir = tempfile::tempdir().unwrap();
    let mut fragment = make_fragment(0);
    fragment.files[0].file_minor_version = 1;
    fragment.files[0].column_indices = vec![-1, 1].into();
    let config_upsert_values = tree.then(|| {
        std::collections::HashMap::from([(
            lance_table::fragment_metadata::MANIFEST_LAYOUT_KEY.to_string(),
            lance_table::fragment_metadata::MANIFEST_LAYOUT_TREE.to_string(),
        )])
    });
    let error = crate::dataset::write::CommitBuilder::new(dir.path().to_str().unwrap())
        .execute(crate::dataset::transaction::Transaction::new_from_version(
            0,
            Operation::Overwrite {
    ...
    assert!(matches!(error, Error::InvalidInput { .. }), "{error}");
    assert!(
        error.to_string().contains("must have a valid column index"),
        "{error}"
    );
}

#[tokio::test]
async fn maximum_explicit_fragment_id_is_accepted_like_flat() {
    let mut diff = Differential::create(2).await;
    let outcome = diff
        .apply_latest(
            "append_last_id",
            Operation::Append {
                fragments: vec![make_fragment(u64::from(u32::MAX))],
            },
        )
        .await;
    assert!(
    ...
    let outcome = diff
        .apply_latest("append_past_last_id", production_style_append(1))
        .await;
    for result in [outcome.flat, outcome.fragment_metadata] {
        let error = result.unwrap_err();
        assert!(matches!(error, Error::InvalidInput { .. }), "{error}");
...
#[rstest::rstest]
#[case::missing_after_first(vec![0, 99], "99")]
#[case::missing_after_last(vec![1, 99], "99")]
#[case::missing_first(vec![99], "99")]
#[case::empty(vec![], "at least one old fragment")]
#[tokio::test]
async fn rewrite_rejects_missing_fragments_like_flat(
    #[case] old_ids: Vec<u64>,
    #[case] message: &str,
) {
    let mut diff = Differential::create(2).await;
    let is_empty = old_ids.is_empty();
    let outcome = diff
        .apply_latest(
            "rewrite_missing_fragment",
            Operation::Rewrite {
                groups: vec![RewriteGroup {
                    old_fragments: old_ids.into_iter().map(make_fragment).collect(),
                    new_fragments: vec![make_fragment(0)],
                }],
                rewritten_indices: Vec::new(),
                frag_reuse_index: None,
            },
...
/// Kinds of the actions still buffered by the tree side, from the root
/// buffer and the manifest suffix.
async fn buffered_kinds(uri: &str) -> BTreeMap<&'static str, usize> {
    use lance_table::format::pb::fragment_action::Action;
    use lance_table::format::pb::fragment_tree::Root;
    let dataset = crate::dataset::builder::DatasetBuilder::from_uri(uri)
        .load()
        .await
        .unwrap();
    let descriptor = dataset.manifest.fragment_tree.as_ref().unwrap();
    let root_buffer = match &descriptor.root {
        Some(Root::InlineRoot(root)) => root.buffer.as_slice(),
        _ => &[],
    };
    let mut kinds = BTreeMap::new();
    for mutation in descriptor.mutations_since_root.iter().chain(root_buffer) {
        let kind = match mutation
            .action
            .as_ref()
            .and_then(|action| action.action.as_ref())
        {
            Some(Action::UpsertFragment(_)) => "upsert",
            Some(Action::RemoveFragment(_)) => "remove",
            Some(Action::AddDataFile(_)) => "add_data_file",
            Some(Action::RemoveDataFile(_)) => "remove_data_file",
            Some(Action::ReplaceDataFile(_)) => "replace_data_file",
            Some(Action::AddDeletionFile(_)) => "add_deletion_file",
            Some(Action::ClearDeletionFile(_)) => "clear_deletion_file",
            None => "other",
        };
...
/// A `Merge` from the flat side's latest state, with `edit` applied to the
/// fragment map and one column per entry of `columns` added to the schema.
async fn merge_from_latest(
    diff: &Differential,
    columns: &[u32],
    edit: impl FnOnce(&mut BTreeMap<u64, Fragment>),
) -> (u64, Operation) {
    let state = diff.flat_state().await;
    let mut fragments = state.fragments.clone();
    edit(&mut fragments);
    let schema = columns.iter().fold(state.schema.clone(), |schema, column| {
        super::differential::schema_with_column(&schema, *column)
    });
    ...
    (state.version, operation)
}

/// Only an appended file lowers to `AddDataFile`. Anything else about the
/// record, including a file that moved or disappeared, keeps the whole
/// record, and both layouts still agree.
#[rstest::rstest]
#[case::two_appended_files("two_appended_files", BTreeMap::from([("add_data_file", 4)]))]
#[case::reordered_files("reordered_files", BTreeMap::from([("add_data_file", 1), ("upsert", 1)]))]
#[case::replaced_file("replaced_file", BTreeMap::from([("add_data_file", 1), ("upsert", 1)]))]
#[case::removed_file("removed_file", BTreeMap::from([("add_data_file", 1), ("upsert", 1)]))]
#[case::deletion_file_change("deletion_file_change", BTreeMap::from([("add_data_file", 1), ("upsert", 1)]))]
#[case::fresh_fragment("fresh_fragment", BTreeMap::from([("add_data_file", 2), ("upsert", 1)]))]
#[tokio::test]
async fn backfill_lowers_appended_files_and_falls_back_like_flat(
    #[case] scenario: &str,
    #[case] expected: BTreeMap<&'static str, usize>,
) {
    // Two rows per fragment, so deleting one row keeps the fragment.
    let fragments = (0..N)
        .map(|id| {
            let mut fragment = make_fragment(id);
    ...
    let mut diff = Differential::create_with_fragments(fragments).await;
    let v1 = diff.flat_state().await.version;
    let outcome = diff.apply_add_columns("backfill", v1, &[1, 2], 0).await;
    assert!(outcome.both_accepted_and_equal(), "{:?}", outcome.diff);
    let (version, operation) = merge_from_latest(&diff, &[1, 2], |fragments| match scenario {
        "two_appended_files" => {
            let fragment = fragments.get_mut(&1).unwrap();
            fragment.files.push(make_backfill_data_file(1, 1));
            fragment.files.push(make_backfill_data_file(1, 2));
        }
        "reordered_files" => {
            let fragment = fragments.get_mut(&1).unwrap();
            fragment.files.swap(0, 1);
            fragment.files.push(make_backfill_data_file(1, 1));
        }
        "replaced_file" => {
            let fragment = fragments.get_mut(&1).unwrap();
            fragment.files[0] =
                lance_table::fragment_metadata::support::make_replacement_data_file(1, 0);
            fragment.files.push(make_backfill_data_file(1, 1));
        }
        "removed_file" => {
            fragments.get_mut(&1).unwrap().files.truncate(1);
        }
        "deletion_file_change" => {
            let fragment = fragments.get_mut(&1).unwrap();
            fragment.deletion_file = Some(super::differential::deletion_file(5, 1));
            fragment.files.push(make_backfill_data_file(1, 1));
        }
        "fresh_fragment" => {
            let mut fragment = make_fragment(N);
            fragment.files.push(make_backfill_data_file(N, 1));
            fragments.insert(N, fragment);
        }
        other => panic!("unknown scenario {other}"),
    })
    .await;
    let outcome = diff.apply(scenario, version, operation).await;
    assert!(
        outcome.both_accepted_and_equal(),
        "{scenario}: flat={:?} fragment_metadata={:?} diff={:?}",
        outcome.flat.as_ref().err(),
        outcome.fragment_metadata.as_ref().err(),
        outcome.diff
    );
    assert_eq!(buffered_kinds(&diff.fragment_metadata.uri).await, expected);
}

/// A buffered appended file replays correctly under every later action on
/// its fragment and through a drain.
#[rstest::rstest]
#[case::deletion_after_add("deletion_after_add")]
#[case::replace_after_add("replace_after_add")]
#[case::add_after_upsert("add_after_upsert")]
#[case::whole_table_add_then_drain("whole_table_add_then_drain")]
#[case::whole_table_add_over_budget("whole_table_add_over_budget")]
#[tokio::test]
async fn buffered_file_additions_replay_like_flat(#[case] scenario: &str) {
    let mut diff = Differential::create(N).await;
    let version = diff.flat_state().await.version;
    let steps: Vec<(&str, u64, Operation)> = match scenario {
        "deletion_after_add" => {
            let outcome = diff.apply_add_columns("add", version, &[1, 2], 0).await;
            assert!(outcome.both_accepted_and_equal(), "{:?}", outcome.diff);
            let state = diff.flat_state().await;
            vec![(
                "deletion",
                state.version,
                set_deletion_file(state.fragments[&1].clone(), 5),
            )]
        }
        "replace_after_add" => {
            let outcome = diff.apply_add_columns("add", version, &[1, 2], 0).await;
            assert!(outcome.both_accepted_and_equal(), "{:?}", outcome.diff);
            let state = diff.flat_state().await;
            vec![("replace", state.version, replace_base([1], 0))]
        }
        "add_after_upsert" => {
            let outcome = diff
                .apply("append", version, production_style_append(1))
                .await;
            assert!(outcome.both_accepted_and_equal(), "{:?}", outcome.diff);
            let state = diff.flat_state().await;
            let outcome = diff.apply_add_columns("add", state.version, &[N], 0).await;
            assert!(outcome.both_accepted_and_equal(), "{:?}", outcome.diff);
            Vec::new()
        }
        "whole_table_add_then_drain" => {
            // The fixture's 4 KiB nodes put the root under structural pressure
            // after one whole-table addition, which drains it into the interior
            // nodes. A one-action bucket does not pay for a leaf rewrite, so
            // more files per fragment are needed before the leaves are
            // rewritten. Every round stays under the semantic budget, so each
            // one lowers to appended files.
            let ids: Vec<u64> = (0..N).collect();
            for column in 0..4 {
                let version = diff.flat_state().await.version;
                let outcome = diff
                    .apply_add_columns(&format!("add_all_{column}"), version, &ids, column)
                    .await;
                assert!(outcome.both_accepted_and_equal(), "{:?}", outcome.diff);
                let kinds = buffered_kinds(&diff.fragment_metadata.uri).await;
                assert!(!kinds.contains_key("upsert"), "{kinds:?}");
                let reader = diff.fragment_metadata_reader().await;
                let shape = reader.tree.shape_report().await.unwrap();
                assert!(
                    shape.root_buffer_len < N,
                    "{} left in the root",
                    shape.root_buffer_len
                );
            }
            let reader = diff.fragment_metadata_reader().await;
            let buffered = reader.tree.buffered_action_keys().await.unwrap();
            assert!(
                buffered.len() < N as usize,
                "{} fragments still have buffered actions",
                buffered.len()
            );
            Vec::new()
        }
        "whole_table_add_over_budget" => {
            // Over the semantic budget the commit cannot stay buffered, so it
            // keeps whole records and their header-only drain.
            for uri in [&diff.flat.uri, &diff.fragment_metadata.uri] {
                let mut dataset = crate::dataset::builder::DatasetBuilder::from_uri(uri)
                    .load()
                    .await
                    .unwrap();
                dataset
                    .update_config([("lance.fragment_metadata.semantic_buffer_bytes", "2048")])
                    .await
                    .unwrap();
            }
            let version = diff.flat_state().await.version;
            let ids: Vec<u64> = (0..N).collect();
            let outcome = diff.apply_add_columns("add_all", version, &ids, 0).await;
            assert!(outcome.both_accepted_and_equal(), "{:?}", outcome.diff);
            let kinds = buffered_kinds(&diff.fragment_metadata.uri).await;
            assert!(!kinds.contains_key("add_data_file"), "{kinds:?}");
            Vec::new()
        }
        other => panic!("unknown scenario {other}"),
    };
    for (step, version, operation) in steps {
        let outcome = diff.apply(step, version, operation).await;
        assert!(
            outcome.both_accepted_and_equal(),
            "{scenario} {step}: flat={:?} fragment_metadata={:?} diff={:?}",
            outcome.flat.as_ref().err(),
            outcome.fragment_metadata.as_ref().err(),
            outcome.diff
        );
...
#[tokio::test]
async fn buffered_file_addition_then_partial_deletion_matches_flat() {
    let fragments = (0..2)
        .map(|id| make_fragment(id).with_physical_rows(2))
        .collect();
    let mut diff = Differential::create_with_fragments(fragments).await;
    let initial = diff.flat_state().await;
    let mut follower = DatasetBuilder::from_uri(&diff.fragment_metadata.uri)
        .load_unmaterialized()
        .await
        .unwrap();
    assert_eq!(follower.get_fragments_async().await.unwrap().len(), 2);

    let added = diff
        .apply_add_columns("add", initial.version, &[1], 0)
        .await;
    assert!(added.both_accepted_and_equal(), "{:?}", added.diff);
    assert_eq!(
        buffered_kinds(&diff.fragment_metadata.uri).await,
        BTreeMap::from([("add_data_file", 1)])
    );
    let after_add = diff.flat_state().await;
    let deleted = diff
        .apply_latest(
            "partial_delete",
            set_deletion_file(after_add.fragments[&1].clone(), 5),
        )
        .await;
    assert!(deleted.both_accepted_and_equal(), "{:?}", deleted.diff);
    let expected = diff.flat_state().await;
    let fragment = &expected.fragments[&1];
    assert_eq!(
        fragment.files,
        vec![
            initial.fragments[&1].files[0].clone(),
            make_backfill_data_file(1, 0)
        ]
    );
    assert_eq!(fragment.physical_rows, Some(2));
    assert_eq!(fragment.num_rows(), Some(1));
    assert_eq!(
        fragment.deletion_file,
        Some(super::differential::deletion_file(5, 1))
    );

    let buffered = DatasetBuilder::from_uri(&diff.fragment_metadata.uri)
        .load_unmaterialized()
        .await
        .unwrap();
    let descriptor = buffered.manifest.fragment_tree.as_ref().unwrap();
    let Some(Root::InlineRoot(root)) = &descriptor.root else {
        panic!("the two-fragment fixture should keep its root inline");
    };
    let mut mutations: Vec<_> = root
        .buffer
        .iter()
        .chain(&descriptor.mutations_since_root)
        .cloned()
        .collect();
    mutations.sort_by_key(|mutation| mutation.action_sequence);
    let actions: Vec<_> = mutations
        .iter()
        .map(|mutation| mutation.action.as_ref().unwrap().action.as_ref().unwrap())
        .collect();
    assert!(matches!(
        actions.as_slice(),
        [Action::AddDataFile(add), Action::AddDeletionFile(delete)]
            if add.frag_id == 1 && delete.frag_id == 1
    ));
    assert!(mutations[0].action_sequence < mutations[1].action_sequence);
    for actions in [mutations.clone(), squash_buffer(mutations)] {
        let deltas: Vec<_> = actions
            .iter()
            .map(|action| {
                (
                    action.fragment_count_delta,
                    action.total_rows_delta,
                    action.visible_rows_delta,
                )
            })
            .collect();
        assert_eq!(deltas, vec![(0, 0, 0), (0, 0, -1)]);
        let mut replayed = initial.fragments.clone();
        apply_verified(&mut replayed, actions).unwrap();
        assert_eq!(replayed, expected.fragments);
    }

    let expected_fragments: Vec<_> = expected.fragments.values().cloned().collect();
    for drain in [false, true] {
        if drain {
            // The second commit uses the bulk policy set by the first.
            for uri in [&diff.flat.uri, &diff.fragment_metadata.uri] {
                ...
                dataset
                    ...
                    .await
                    .unwrap();
                dataset.update_config([("test.drain", "1")]).await.unwrap();
            }
        }
        assert!(diff.states_equal().await);
        assert_eq!(diff.flat_state().await.fragments, expected.fragments);
        let reader = diff.fragment_metadata_reader().await;
        assert_eq!(reader.tree.count_fragments(), 2);
        assert_eq!(reader.tree.count_rows(), 4);
        assert_eq!(reader.tree.count_visible_rows(), 3);
        assert_eq!(
            reader.resolve_fragment(1).await.unwrap().as_ref(),
            Some(fragment)
        );
        assert_eq!(reader.materialize().await.unwrap(), expected_fragments);
        if drain {
            assert!(reader.tree.buffered_action_keys().await.unwrap().is_empty());
        }
        follower.checkout_latest().await.unwrap();
        let followed: Vec<_> = follower
            .get_fragments_async()
            .await
            .unwrap()
            .into_iter()
            ...
            .collect();
        assert_eq!(followed, expected_fragments);
        assert_eq!(follower.count_rows(None).await.unwrap(), 3);
    }
    let historical =
        super::test_support::Reader::open_uri_at(&diff.fragment_metadata.uri, expected.version)
            .await
            .unwrap();
    assert_eq!(historical.materialize().await.unwrap(), expected_fragments);
}
