...
//! snapshot: an inline root, or `_bt/root/{uuid}.root` plus
...
use lance_table::format::{Manifest, pb};
use lance_table::fragment_metadata::store::NodeStore;
use lance_table::fragment_metadata::{
    FragmentTree, FragmentTreeConfig, ManifestLayout, TouchedFragments, data_replacement,
};
...
impl FragmentMetadataOptions {
    ...
    /// Layout and the hard object limit cannot change on an existing dataset.
...
pub(crate) fn dataset_uses_fragment_metadata(manifest: &Manifest) -> bool {
    manifest.fragment_tree.is_some()
}
...
pub(crate) async fn execute_create(
    ...
    super::versions::finalize_manifest_storage_version(&mut manifest)?;
    // Tree aggregates require known counts. Recover missing deletion counts;
    // the tree builder rejects missing physical row counts.
...
pub(crate) async fn execute_commit_with_timeout(
    ...
    for attempt in 1..=attempts {
        let mut dataset = publication::open_latest(&target).await?;
        lance_table::feature_flags::ensure_can_write_manifest(&dataset.manifest)?;
        let lazy = dataset.lazy_fragments.as_ref().ok_or_else(|| {
            Error::invalid_input("Dataset metadata layout changed since this handle was opened")
        })?;
        ...
        if let Some(context) = target.context.as_ref().filter(|context| {
            context.lazy_fragments.is_none()
                ...
                && context.manifest.fragment_tree == dataset.manifest.fragment_tree
        ...
        let mut rebased = Cow::Borrowed(transaction);
        ...
        if strict_overwrite && transaction.read_version != tree.version() {
            return Err(Error::commit_conflict_source(
                ...
                format!(
                    "Strict overwrite expected version {}, but current version is {}",
                    transaction.read_version,
                    tree.version()
                )
                .into(),
                ...
                && matches!(transaction.operation, Operation::Overwrite { .. }))
        {
            ...
            rebased = Cow::Owned(resolver.finish(&dataset).await?);
        }
        ...
        let mut prepared = native::prepare(
            &dataset,
            &mut tree,
            &rebased,
            transaction.read_version,
...
/// Copy the root into the destination and stamp inherited references with
/// `source_base_id`. Shared descendants stay in the source dataset.
pub(crate) async fn clone_fragment_metadata(
    ...
    let mut root = match &snapshot.root {
        Some(pb::fragment_tree::Root::InlineRoot(root)) => root.clone(),
        Some(pb::fragment_tree::Root::RootUuid(uuid)) => source_nodes.read_root_base(uuid).await?,
        None => {
            return Err(Error::invalid_input(
    ...
    let (uuid, _) = dest_nodes.write_root_base(&root).await?;
    manifest.fragment_tree = Some(Arc::new(pb::FragmentTree {
        root: Some(pb::fragment_tree::Root::RootUuid(uuid)),
...
fn tree_config_from(config: &HashMap<String, String>) -> Result<FragmentTreeConfig> {
    if let Some(materialization) = config.get("lance.fragment_metadata.materialization")
        && !["buffered", "bulk"].contains(&materialization.as_str())
    {
        return Err(Error::invalid_input(format!(
            "lance.fragment_metadata.materialization must be one of [\"buffered\", \"bulk\"], \
             got {materialization:?}"
        )));
    }
        ...
        .filter(|key| key.starts_with("lance.fragment_metadata."))
    {
        if !matches!(
            ...
            MAX_NODE_BYTES_KEY
                ...
                // Older writers persisted this gate; tree height is now unrestricted.
    ...
    let mut tree_config = FragmentTreeConfig::default();
