...
pub(crate) fn aggregate_deltas(
    ...
    for (offset, fragment_action) in actions.iter().enumerate() {
        ...
        // count alone, so the record is not copied to apply it. Replay
        // applies and checks the file at its leaf.
        if let Some(Action::AddDataFile(_)) = &fragment_action.action
            && last_for_fragment
        {
        ...
        // is not decoded here. Replay decodes and validates it at its leaf.
...
mod tests {
    ...
    fn aggregate_deltas_keep_counts_for_a_last_add_data_file() {
        ...
        let deltas = aggregate_deltas(&[action::add_data_file(3, &file)], &touched, 10).unwrap();
        ...
        let error = aggregate_deltas(&[action::add_data_file(9, &file)], &touched, 10).unwrap_err();
        ...
        let error = aggregate_deltas(&[empty], &touched, 10).unwrap_err();
    ...
    fn aggregate_deltas_reject_unresolved_mutations_and_stale_appends() {
        ...
        let error = aggregate_deltas(&[action::upsert_fragment(&make_fragment(3))], &touched, 10)
            .unwrap_err();
        assert!(error.to_string().contains("without resolving"), "{error}");

        let deltas =
            aggregate_deltas(&[action::upsert_fragment(&make_fragment(10))], &touched, 10).unwrap();
        assert_eq!(deltas.len(), 1);
        ...
        let deltas = aggregate_deltas(&[action::upsert_fragment(&empty)], &touched, 10).unwrap();
